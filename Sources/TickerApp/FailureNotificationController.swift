...
final class FailureNotificationController: NSObject, FailureNotificationHandling, UNUserNotificationCenterDelegate {
            ...
            DispatchQueue.main.async {
                ...
                case .authorized, .provisional, .ephemeral:
                    if self.hasReceivedJobSnapshot {
                        self.reconcile(deliver: true)
                    }
                case .notDetermined:
                    self.requestAuthorization()
                case .denied:
                    if self.hasReceivedJobSnapshot {
                        self.reconcile(deliver: false)
                    }
                @unknown default:
                    if self.hasReceivedJobSnapshot {
                        self.reconcile(deliver: false)
    ...
    private func reconcile(deliver: Bool) {
    ...
    private func notificationRequest(
        ...
        return UNNotificationRequest(
            identifier: "ticker-attention-\(Self.digest(identity))",
            content: content,
            trigger: nil
        )
    }
